...
def get_quit_members(current_members_data: Dict[int, List[Dict]]) -> Dict[int, List[int]]:
    """
    计算退群人员
    
    Args:
        current_members_data: 键为群号,值为该群当前成员列表的字典
    """
    quit_members_dict = {}
    ...
    for group_id in config['group_ids']:
        ...
        current_members = {member['user_id'] for member in current_members_data[group_id]}
        quit_members = historical_members - current_members
        quit_members_dict[group_id] = list(quit_members)
    
    return quit_members_dict

def get_new_members(current_members_data: Dict[int, List[Dict]]) -> Dict[int, List[int]]:
    """
    计算新入群人员
